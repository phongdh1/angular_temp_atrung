...
export class BaseApiService {
  ...
  deleteSingle<T>(uri: string, id: string): Observable<T> {
    return this.http.delete<T>(environment.apiUrl + uri +"/" + id);
  }
  ...
  getQueryString<T>(uri: string, key?: any, format?: string, id?: string): Observable<T> {
    return this.http.get<T>(environment.apiUrl + uri + "/" + id + "/?" + key + "=" + format);
  }
  downloadFile(uri: string, data?: any): Observable<any>{
    return this.http.post(environment.apiUrl + uri, data, { observe: 'response', responseType: 'blob' } );
  }
